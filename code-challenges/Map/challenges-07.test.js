...
const forEachTwoToThe = (arr) => {
  // Solution code here...
  arr.forEach((ele,idx)=> arr[idx] = Math.pow(2,ele));
  return arr;
};
/* ------------------------------------------------------------------------------------------------
CHALLENGE 3
...
const mapTwoToThe = (arr) => {
  // Solution code here...
  arr = arr.map(ele => Math.pow(2,ele));
  return arr;
};
...
const charCode = (arr) => {
  // Solution code here...
  arr = arr.map(ele=> ele =ele.charCodeAt(0));
  return arr;
};
...
const extractStats = (arr) => {
  ...
  let result = [];
  arr.map(ele => {
    let total = ele.effort + ele.baseStat;
    let obj = {name: ele.stat.name,total: total};
    result.push(obj);
  });
  return result;
};
