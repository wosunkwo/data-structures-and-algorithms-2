...
const toTitleCase = (arr) => arr.map( ele => ele.charAt(0).toUpperCase()+ ele.substring(1,ele.length));
  // Solution code here..
...
let biggerThanLuke = (arr) => {
  ...
  let result = '';
  arr.forEach(ele =>{
    if(parseInt(ele.mass) > parseInt(arr[0].mass)){
      result += ele.name + ' - ';
    }
  });
...
const sortBy = (property, arr) => arr.sort( (a,b) => (property === 'name')? a.name.localeCompare(b.name) : a.price - b.price);
