...
const totalSum = (input) => {
  // Solution code here...
  let result = input.reduce ( (sum, curr) => {
    curr = curr.reduce( (innerSum, innerCurr) => innerSum + innerCurr);
    return sum + curr;
  },0);
  return result;
};
...
const divisibleByFiveTwoToThePower = (input) => {
  // Solution code here...
  let results = input.map( ele =>{
    let result = ele.filter( innerEle =>{
      if( typeof(innerEle) === 'number'){
        if(innerEle % 5 === 0){
          return 1;
        }
      }
    });
    return result.map( innerEle => Math.pow(2,innerEle));
  });
  return results;
};
...
let findMaleAndFemale = (data) => {
  ...
  let newResult = newData.map( ele => ele.name);
...
let findShortest = (data) => {
  // Solution code here...
  let result = data.reduce( (sum, curr) => sum.height < curr.height ? curr : sum);
  return result.name;
};
